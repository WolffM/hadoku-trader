...
function loadSignalsTyped(): RawSignal[] {
  return loadSignalsFromExport().filter((s: RawSignal) =>
    s.ticker &&
    s.trade_date &&
    s.trade_price > 0 &&
    s.action &&
...
describe("Portfolio Simulation", () => {
  ...
  it("should compare ALL strategies over 3-year training set", () => {
    ...
    for (let i = 0; i < sortedResults.length; i++) {
      ...
      console.log(`${badge} ${r.name.padEnd(20)} ${formatPct(r.growthPct).padStart(8)} growth → ${formatMoney(r.portfolioValue)} portfolio`);
    }
    ...
    for (let i = 0; i < 10 && i < qualified.length; i++) {
      const p = qualified[i];
      console.log(`  ${(i + 1).toString().padStart(2)}. ${p.name.padEnd(25)} ${p.party} | ${p.trades} trades | Ann: ${formatPct(p.annualizedReturnPct)}`);
    }
  ...
  it("should correctly simulate Nancy Pelosi trades using production code", () => {
    ...
    console.log(`Portfolio value: $${(result.finalCash + result.openPositions.reduce((s, p) => s + p.cost, 0)).toFixed(0)}`);
