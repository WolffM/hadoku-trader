...
function runAgentVerification(config: AgentConfig, signals: SimSignal[]): AgentResult {
  ...
  for (const simSignal of validSignals) {
    ...
    // BUY signal
      ...
      if (config.politician_whitelist && !config.politician_whitelist.includes(simSignal.politician_name)) {
...
function printDecisionLog(result: AgentResult): void {
  ...
  // Summary
  console.log("\n" + "-".repeat(180));
  console.log(`SUMMARY: ${result.totalSignals} signals | ${result.executed} executed | ${result.skipped} skipped`);
  console.log(`Final: $${result.finalCash.toFixed(0)} cash, ${result.openPositions.length} open positions`);
...
function printScoringBreakdowns(result: AgentResult): void {
  ...
  console.log("\n" + "═".repeat(100));
  console.log(`SCORING BREAKDOWNS: ${result.agentName} (${executedDecisions.length} executed trades)`);
  console.log("═".repeat(100));

  for (const d of executedDecisions) {
    const b = d.breakdown!;
    console.log(`\nSignal: ${d.signalId} (${d.ticker}, ${d.politician}, ${d.signalAction})`);
    console.log(`  Days since trade: ${d.daysSinceTrade}, Price change: ${formatPct(d.priceChangePct)}`);
    console.log(`  ─────────────────────────────────────────────────────────`);

    if (b.time_decay.weight > 0) {
      console.log(`  time_decay:       ${b.time_decay.raw.toFixed(3)} × ${b.time_decay.weight.toFixed(2)} = ${b.time_decay.contribution.toFixed(4)}`);
    }
    if (b.price_movement.weight > 0) {
      console.log(`  price_movement:   ${b.price_movement.raw.toFixed(3)} × ${b.price_movement.weight.toFixed(2)} = ${b.price_movement.contribution.toFixed(4)}`);
    }
    if (b.position_size.weight > 0) {
      console.log(`  position_size:    ${b.position_size.raw.toFixed(3)} × ${b.position_size.weight.toFixed(2)} = ${b.position_size.contribution.toFixed(4)}`);
    }
    if (b.politician_skill.weight > 0) {
      console.log(`  politician_skill: ${b.politician_skill.raw.toFixed(3)} × ${b.politician_skill.weight.toFixed(2)} = ${b.politician_skill.contribution.toFixed(4)}`);
    }
    if (b.source_quality.weight > 0) {
      console.log(`  source_quality:   ${b.source_quality.raw.toFixed(3)} × ${b.source_quality.weight.toFixed(2)} = ${b.source_quality.contribution.toFixed(4)}`);
    }
    if (b.filing_speed && b.filing_speed.weight > 0) {
      console.log(`  filing_speed:     ${b.filing_speed.raw.toFixed(3)} × ${b.filing_speed.weight.toFixed(2)} = ${b.filing_speed.contribution.toFixed(4)}`);
    }
    if (b.cross_confirmation && b.cross_confirmation.weight > 0) {
      console.log(`  cross_confirm:    ${b.cross_confirmation.raw.toFixed(3)} × ${b.cross_confirmation.weight.toFixed(2)} = ${b.cross_confirmation.contribution.toFixed(4)}`);
    }

    console.log(`  ─────────────────────────────────────────────────────────`);
    console.log(`  FINAL SCORE: ${b.final_score.toFixed(4)} (threshold=${result.agentId === "chatgpt" ? 0.55 : 0.55})`);
...
function printCondensedTradesReport(result: AgentResult): void {
  ...
  console.log("\n" + "═".repeat(160));
  console.log(`EXECUTED TRADES: ${result.agentName} (${result.agentId}) - ${executedTrades.length} trades`);
  ...
  for (const r of results) {
    if (r.openPositions.length === 0) {
      ...
      for (const p of r.openPositions) {
        console.log(`  ${p.ticker}: ${p.shares.toFixed(3)} shares @ $${p.entryPrice.toFixed(2)} = $${p.cost.toFixed(2)}`);
      }
    }
  }
}
...
describe("45-Day Signal Verification", () => {
  it("should run ALL agents and produce verification report", () => {
    ...
    console.log(`Date range: ${signals[0]?.disclosure_date || "N/A"} to ${signals[signals.length - 1]?.disclosure_date || "N/A"}`);
  ...
  it("should produce condensed trades report (ChatGPT only)", () => {
    ...
    for (const p of result.openPositions) {
      console.log(`  ${pad(p.ticker, 6, true)} | ${p.shares.toFixed(4)} shares @ $${p.entryPrice.toFixed(2)} = $${p.cost.toFixed(2)}`);
    }
