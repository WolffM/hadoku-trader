...
import type { AgentPortfolio, PerformanceMetrics, CloseReason as _CloseReason } from './types'
...
export function calculateMetrics(portfolio: AgentPortfolio): PerformanceMetrics {
  ...
  const { totalTrades, winRate, avgWinPct, avgLossPct, avgHoldDays } =
...
function calculateReturns(portfolio: AgentPortfolio): {
  ...
  if (yearsTraded > 0 && totalReturnPct > -100) {
    ...
    annualizedReturnPct = (Math.pow(totalReturn, 1 / yearsTraded) - 1) * 100
  }
 ...
 */
function calculateMaxDrawdown(snapshots: AgentPortfolio['dailySnapshots']): number {
  if (snapshots.length === 0) {
    return 0
  }
 ...
 */
function calculateTradeStats(closedPositions: AgentPortfolio['closedPositions']): TradeStats {
  ...
  const tradeReturns = trades.map(trade => ({
    returnPct: ((trade.closePrice! - trade.entryPrice) / trade.entryPrice) * 100,
    holdDays: trade.closeDate ? daysBetween(trade.entryDate, trade.closeDate) : 0
  }))
  ...
  const avgWinPct =
    winners.length > 0 ? winners.reduce((sum, t) => sum + t.returnPct, 0) / winners.length : 0

  const avgLossPct =
    losers.length > 0 ? losers.reduce((sum, t) => sum + t.returnPct, 0) / losers.length : 0

  const avgHoldDays = tradeReturns.reduce((sum, t) => sum + t.holdDays, 0) / totalTrades
...
export function calculateInformationRatio(
  ...
  if (agentReturns.length !== benchmarkReturns.length || agentReturns.length < 2) {
    return 0
  }
  ...
  // Mean active return
  const meanActive = activeReturns.reduce((sum, r) => sum + r, 0) / activeReturns.length
 ...
 */
export function calculateSortinoRatio(dailyReturns: number[], targetReturn = 0): number {
  if (dailyReturns.length < 2) {
    return 0
  }
