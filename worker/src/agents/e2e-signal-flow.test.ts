              ...
              for (const row of tableData) {
                ...
                for (const cond of conditions) {
                  ...
                  if (colMatch) {
                    ...
                    if (nullColMatch && row[nullColMatch[1]] !== null && row[nullColMatch[1]] !== undefined) {
                      matches = false;
                      break;
                    }
                  }
...
function createMockEnv(): TraderEnv & { mockDb: ReturnType<typeof createMockD1> } {
  ...
  mockDb.tables.agents = [
    { id: "chatgpt", name: "Decay Edge", is_active: 1, config_json: JSON.stringify(CHATGPT_CONFIG) },
    { id: "claude", name: "Decay Alpha", is_active: 1, config_json: JSON.stringify(CLAUDE_CONFIG) },
    { id: "gemini", name: "Titan Conviction", is_active: 1, config_json: JSON.stringify(GEMINI_CONFIG) },
  ];
...
describe("End-to-End Signal Flow", () => {
  ...
  describe("Signal Enrichment", () => {
    it("should correctly enrich raw signal data for scoring", () => {
      ...
      expect(enriched.current_price).toBe(145.00);
  ...
  describe("Scoring System", () => {
    it("should calculate score components correctly", async () => {
      ...
      const currentPrice = 142.80; // 2% move since disclosure
      const enriched = enrichSignal(rawSignal, currentPrice);
    ...
    it("should penalize stale signals", async () => {
      ...
      const freshSignal = createRawSignal({
        ...
        disclosure_date: new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
      });
      ...
      const staleSignal = createRawSignal({
        ...
        disclosure_date: new Date(now.getTime() - 40 * 24 * 60 * 60 * 1000).toISOString().split("T")[0],
      });

      const currentPrice = 142.00;
      ...
      expect(freshScore.breakdown?.time_decay).toBeGreaterThan(staleScore.breakdown?.time_decay || 0);
      // Overall score may be affected by other factors, just check time_decay

      console.log("Fresh vs Stale:", {
        fresh: { score: freshScore.score.toFixed(3), time_decay: freshScore.breakdown?.time_decay?.toFixed(3), days: freshEnriched.days_since_disclosure },
        stale: { score: staleScore.score.toFixed(3), time_decay: staleScore.breakdown?.time_decay?.toFixed(3), days: staleEnriched.days_since_disclosure },
      });
    });
    ...
    it("should reward larger congressional positions", async () => {
      ...
      const smallScore = await calculateScore(env, CHATGPT_CONFIG.scoring!, enrichSignal(smallSignal, currentPrice));
      const largeScore = await calculateScore(env, CHATGPT_CONFIG.scoring!, enrichSignal(largeSignal, currentPrice));

      // Large positions should have higher position_size component
      expect(largeScore.breakdown?.position_size).toBeGreaterThan(smallScore.breakdown?.position_size || 0);

      console.log("Small vs Large position:", {
        small: { score: smallScore.score.toFixed(3), position_size: smallScore.breakdown?.position_size?.toFixed(3) },
        large: { score: largeScore.score.toFixed(3), position_size: largeScore.breakdown?.position_size?.toFixed(3) },
      });
    });
  });
  ...
  describe("Position Sizing", () => {
    it("should calculate position size based on score and mode", () => {
      ...
      const sizeLinear = calculatePositionSize(
        CLAUDE_CONFIG,
        0.8,
        budget,
        1,
  ...
  describe("Full Pipeline Integration", () => {
    ...
    it("should skip non-whitelisted politicians for Gemini", async () => {
      ...
      const decisions = await routeSignalToAgents(env, rawSignal, 145.00, false);
  ...
  describe("Decision Threshold Behavior", () => {
    it("should execute when score >= execute_threshold", async () => {
      ...
      const rawSignal = createRawSignal({
        ...
        disclosure_date: new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000).toISOString().split("T")[0], // Very fresh
        position_size_min: 500001, // Large position = high conviction
        source: "quiver_quant", // Best source
      });

      const currentPrice = 140.50; // Small move (good)
      const enriched = enrichSignal(rawSignal, currentPrice);
    ...
    it("should execute_half when score between thresholds", async () => {
      ...
      const rawSignal = createRawSignal({
        disclosure_date: new Date(Date.now() - 20 * 24 * 60 * 60 * 1000).toISOString().split("T")[0], // Moderately stale
        position_size_min: 20000, // Small-medium position
        source: "house_stock_watcher", // Lower quality source (0.8)
      });

      const currentPrice = 150.00; // Larger move
      const enriched = enrichSignal(rawSignal, currentPrice);
...
describe("Async Signal Provider Simulation", () => {
  ...
  it("should handle rapid signal ingestion", async () => {
    ...
    const allDecisions = [];
    for (const signal of signals) {
      const decisions = await routeSignalToAgents(env, signal, 145.00, false);
  ...
  it("should correctly track budget across multiple signals", async () => {
    ...
    for (const signal of signals) {
      await routeSignalToAgents(env, signal, 145.00, false);
    }
...
describe("SELL Signal Handling", () => {
  it("should skip SELL signals when no position exists", async () => {
    ...
    const decisions = await routeSignalToAgents(env, sellSignal, 145.00, false);
    ...
    console.log("SELL without position:", decisions.map(d => ({
      agent: d.agent_id,
      action: d.action,
