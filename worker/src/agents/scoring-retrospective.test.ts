  ...
  for (const [_key, group] of groups) {
    ...
    for (const buy of group.buys) {
      ...
      if (sell && sell.disclosure_price && buy.disclosure_price) {
        ...
        const returnPct = ((sell.disclosure_price - buy.disclosure_price) / buy.disclosure_price) * 100;
...
describe("Scoring Algorithm Retrospective", () => {
  ...
  it("should analyze returns by score bucket", () => {
    ...
    console.log("\n" + pad("Score Range", 15, true) + " | " + pad("Count", 6) + " | " + pad("Avg Return", 12) + " | " + pad("Win Rate", 10));
  ...
  it("should analyze returns by days since trade", () => {
    ...
    const dayData = tradesWithScores.map(t => ({ input: t.daysSinceTrade, return: t.trade.returnPct }));
    const dayAnalysis = bucketAnalysis(dayData, dayBuckets);

    console.log("\n" + pad("Days Range", 15, true) + " | " + pad("Count", 6) + " | " + pad("Avg Return", 12) + " | " + pad("Win Rate", 10));
    console.log("-".repeat(55));
  ...
  it("should analyze returns by price movement since trade", () => {
    ...
    const pctData = tradesWithScores.map(t => ({ input: t.priceChangePct, return: t.trade.returnPct }));
    const pctAnalysis = bucketAnalysis(pctData, pctBuckets);

    console.log("\n" + pad("Price Δ%", 15, true) + " | " + pad("Count", 6) + " | " + pad("Avg Return", 12) + " | " + pad("Win Rate", 10));
    console.log("-".repeat(55));
    ...
    if (dips.length > 0) {
      ...
      console.log(`  Dips (price dropped):    ${dips.length} trades, ${formatPct(dipAvg)} avg, ${dipWin.toFixed(0)}% win rate`);
    }
    if (gains.length > 0) {
      ...
      console.log(`  Gains (price rose):      ${gains.length} trades, ${formatPct(gainAvg)} avg, ${gainWin.toFixed(0)}% win rate`);
    }
    if (flat.length > 0) {
      ...
      console.log(`  Flat (±2%):              ${flat.length} trades, ${formatPct(flatAvg)} avg, ${flatWin.toFixed(0)}% win rate`);
    }
  ...
  it("should analyze returns by congressional position size", () => {
    ...
    const sizeData = tradesWithScores.map(t => ({ input: t.positionSize, return: t.trade.returnPct }));
    const sizeAnalysis = bucketAnalysis(sizeData, sizeBuckets);

    console.log("\n" + pad("Size Range", 20, true) + " | " + pad("Count", 6) + " | " + pad("Avg Return", 12) + " | " + pad("Win Rate", 10));
  ...
  it("should test alternative weight configurations", () => {
    ...
    const configs = [
      { name: "Current (ChatGPT)", time: 0.30, price: 0.25, size: 0.15, skill: 0.20, source: 0.10 },
      { name: "Time Heavy", time: 0.50, price: 0.20, size: 0.10, skill: 0.15, source: 0.05 },
      { name: "Price Heavy", time: 0.20, price: 0.45, size: 0.10, skill: 0.15, source: 0.10 },
      { name: "Size Heavy", time: 0.20, price: 0.20, size: 0.35, skill: 0.15, source: 0.10 },
      { name: "Skill Heavy", time: 0.20, price: 0.20, size: 0.10, skill: 0.40, source: 0.10 },
      { name: "Equal Weights", time: 0.20, price: 0.20, size: 0.20, skill: 0.20, source: 0.20 },
      { name: "Time+Price", time: 0.40, price: 0.40, size: 0.05, skill: 0.10, source: 0.05 },
      { name: "Fresh Only", time: 0.60, price: 0.20, size: 0.05, skill: 0.10, source: 0.05 },
    ];
    ...
    console.log("\n" + pad("Config", 18, true) + " | " + pad("Trades", 7) + " | " + pad("Avg Return", 12) + " | " + pad("Win Rate", 10) + " | " + pad("Sharpe", 8));
  ...
  it("should find optimal threshold for execution", () => {
    ...
    console.log("\n" + pad("Threshold", 10) + " | " + pad("Trades", 7) + " | " + pad("Avg Return", 12) + " | " + pad("Win Rate", 10) + " | " + pad("Total Return", 14));
  ...
  it("should analyze top politician performance vs scoring", () => {
    ...
    console.log("\n" + pad("Politician", 25, true) + " | " + pad("Trades", 7) + " | " + pad("Avg Return", 12) + " | " + pad("Win%", 6) + " | " + pad("Avg Score", 10) + " | " + pad("Score Corr", 11));
    ...
    console.log("(Positive = scoring identifies better politicians, Negative = scoring is misleading)");
