...
function calculatePoliticianStats(
  ...
  for (const [ticker, tickerSigs] of tickerSignals) {
    ...
    for (const position of positionQueue) {
      ...
      if (latestPrice) {
        ...
        position.unrealizedPct = ((latestPrice.price - position.entryPrice) / position.entryPrice) * 100;
      }
    }
  ...
  const avgCongressionalSize = buySignals.length > 0 ? totalCongressionalSize / buySignals.length : 0;
...
function calculateStrategyStats(
  ...
  const totalPnL = realizedPnL + unrealizedPnL;
  const totalCostWithPrices = filtered.reduce(
    (sum, p) => {
  ...
  const actualTotalCost = filtered.reduce((sum, p) => {
    ...
    const closedCost = p.closedTrades > 0 && p.realizedReturnPct !== 0
  ...
  const totalHoldDaysWeighted = filtered.reduce(
    (sum, p) => sum + (p.avgHoldDays * p.closedTrades), 0
  );
  const avgHoldDays = totalClosed > 0 ? totalHoldDaysWeighted / totalClosed : 0;

  // Weighted win rate
  const totalWins = filtered.reduce(
    (sum, p) => sum + Math.round((p.winRate / 100) * p.closedTrades), 0
  );
  ...
  const totalCongressionalSize = filtered.reduce((sum, p) => sum + (p.avgCongressionalSize * p.trades), 0);
...
describe("Politician Performance Analysis", () => {
  it("should calculate politician and strategy returns (realized + unrealized)", () => {
    ...
    const strategies: StrategyStats[] = [
      ...
      calculateStrategyStats(politicianStats, p => p.party === "D" && p.chamber === "house", "D House"),
      calculateStrategyStats(politicianStats, p => p.party === "D" && p.chamber === "senate", "D Senate"),
      calculateStrategyStats(politicianStats, p => p.party === "R" && p.chamber === "house", "R House"),
      calculateStrategyStats(politicianStats, p => p.party === "R" && p.chamber === "senate", "R Senate"),
    ];
    ...
    console.log("\n" + "═".repeat(140));
    console.log("STRATEGY SUMMARY: TOP 10 INDIVIDUAL WINNERS + 8 COLLECTIVE STRATEGIES (sorted by Total P&L)");
    ...
    console.log(`  Winner: ${dems.annualizedReturnPct > reps.annualizedReturnPct ? "Democrats" : "Republicans"} by ${Math.abs(dems.annualizedReturnPct - reps.annualizedReturnPct).toFixed(1)}pp`);
    ...
    console.log(`  Winner: ${house.annualizedReturnPct > senate.annualizedReturnPct ? "House" : "Senate"} by ${Math.abs(house.annualizedReturnPct - senate.annualizedReturnPct).toFixed(1)}pp`);

    // Top performer
    console.log(`\nTop Individual: ${top10[0].name} at ${formatPct(top10[0].annualizedReturnPct)} annualized`);
    console.log(`  ${top10[0].trades} trades, ${top10[0].uniqueTickers} tickers, ${formatMoney(top10[0].totalPnL)} total P&L`);
    ...
    console.log(`  Congress ${vsSPYAnn >= 0 ? "BEATS" : "TRAILS"} SPY by ${Math.abs(vsSPYAnn).toFixed(1)}pp annualized`);

    // Count how many strategies beat SPY (annualized)
    const beatSPY = [dems, reps, house, senate].filter(s => s.annualizedReturnPct > spyAnnualized).length;
  ...
  it("should calculate buy-only portfolio growth (hold forever)", () => {
    ...
    for (const [name, buys] of politicianBuys) {
      ...
      const annualizedReturnPct = avgHoldDays > 0 ? annualizeReturn(returnPct, avgHoldDays) : returnPct;
    ...
    const totalAnnualized = avgHoldDays > 0 ? annualizeReturn(totalReturnPct, avgHoldDays) : totalReturnPct;
  ...
  it("should calculate returns with automatic 1-year exit", () => {
    ...
    const totalRealizedReturnPct = totalRealizedCost > 0 ? (totalRealizedPnL / totalRealizedCost) * 100 : 0;
    ...
    const totalUnrealizedReturnPct = totalUnrealizedCost > 0 ? (totalUnrealizedPnL / totalUnrealizedCost) * 100 : 0;
  ...
  it("should analyze signal volume at different filter thresholds", () => {
    ...
    const sortedByReturn = [...politicianStats].sort((a, b) => b.annualizedReturnPct - a.annualizedReturnPct);
    ...
    results.push(calcFilter("Top 3 (min 15 trades)", p => p.trades >= minTrades && qualified.indexOf(p) < 3));
    results.push(calcFilter("Top 5 (min 15 trades)", p => p.trades >= minTrades && qualified.indexOf(p) < 5));
    results.push(calcFilter("Top 10 (min 15 trades)", p => p.trades >= minTrades && qualified.indexOf(p) < 10));
    results.push(calcFilter("Top 15 (min 15 trades)", p => p.trades >= minTrades && qualified.indexOf(p) < 15));
    results.push(calcFilter("Top 20 (min 15 trades)", p => p.trades >= minTrades && qualified.indexOf(p) < 20));
    results.push(calcFilter("Top 25 (min 15 trades)", p => p.trades >= minTrades && qualified.indexOf(p) < 25));
    results.push(calcFilter("Top 30 (min 15 trades)", p => p.trades >= minTrades && qualified.indexOf(p) < 30));
    ...
    results.push(calcFilter("Ann% >= 20% AND >= 15 trades", p => p.annualizedReturnPct >= 20 && p.trades >= 15));
    results.push(calcFilter("Ann% >= 15% AND >= 30 trades", p => p.annualizedReturnPct >= 15 && p.trades >= 30));
    results.push(calcFilter("Ann% >= 25% AND >= 50 trades", p => p.annualizedReturnPct >= 25 && p.trades >= 50));
    ...
    for (const r of closest.slice(0, 5)) {
      console.log(`  ${r.name}: ${r.signalsPerMonth.toFixed(1)} sig/mo, ${formatPct(r.avgAnnReturn)} avg ann%`);
  ...
  it("should compare politician filters (Top 5/10/15)", () => {
    ...
    console.log("\n" + "═".repeat(140));
    console.log("This is the RAW congressional copy ceiling - buy when they buy, sell when they sell.");
