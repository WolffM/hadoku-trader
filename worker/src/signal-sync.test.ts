      ...
      return {
        ...
        async first(): Promise<MockRow | null> {
          ...
          if (sql.includes('source = ?') && sql.includes('source_id = ?')) {
            ...
            return tableData.find(r => r.source === source && r.source_id === sourceId) || null
          }

          // Handle logical duplicate check (ticker + politician_name + trade_date + action)
          if (
            sql.includes('ticker = ?') &&
            sql.includes('politician_name = ?') &&
            sql.includes('trade_date = ?') &&
            sql.includes('action = ?')
          ) {
            ...
            return (
              tableData.find(
                r =>
                  r.ticker === ticker &&
                  r.politician_name === politicianName &&
...
function toScraperSignal(signal: Signal): components['schemas']['Signal'] {
  return {
    ...
    politician: {
      ...
      chamber:
        (signal.politician.chamber?.toLowerCase() as components['schemas']['Chamber']) ?? 'unknown',
...
describe('syncSignalsFromScraper', () => {
  ...
  it('should skip duplicates when syncing', async () => {
    ...
    const existingSignal = createTestSignal({
      ...
      meta: {
        source_url: '',
        source_id: 'existing_scraper_signal',
        scraped_at: new Date().toISOString()
      }
    })
    await ingestSignalBatch(env, [existingSignal])
  ...
  it('should handle a realistic scraper response', async () => {
    ...
    const realisticSignals = [
      createTestSignal({
        ...
        meta: {
          source_url: 'https://capitoltrades.com/trade/1',
          source_id: 'ct_12345',
      ...
      createTestSignal({
        ...
        meta: {
          source_url: 'https://quiverquant.com/trade/2',
          source_id: 'qq_67890',
    ...
    console.log(
      'Synced signals:',
      env.mockDb.tables.signals.map(s => ({
        ticker: s.ticker,
